...
- Most commonly, we save frames for visualization at about every 200 steps, or about 250ms, which is well into common diffusion regimes.
- But can we go faster? The closer to numerical timestep, the less the flow model must account for integration errors.

SDXS speeds (for single batch) and single-step: 30 FPS or 33ms for 1024x1024 natural images.
- 24ms is UNet, while 6ms is decoder (and 2 ms for text encoder), using a latent diffusion model.

We target 60 FPS (or 16ms) on 2048x2048 images on an L40S GPU, since vorticity is single-channel (grayscale as opposed to RGB).
...
The base denoiser includes:

- a 7x7 convolution to blend tokens with conditioning (position & denoising time) and lift
- an 8x pixelunshuffle (to Lx8x8)
- 6 feedforward MLP/SIREN layers
- an 8x pixel shuffle

In general we use AB2CN stepping (so 2 evaluations per step).

## Metrics

- We use FID and best P90 time for now, need to add frequency spectra and more fluid metrics.
- We calculate FID on the 512x512 image, to avoid distortion introduced by excessive downsampling.

## Select Experiment Log:

1) 5-step pixel diffusion, where each evaluation passes through the autoencoder.
2) 5-step latent diffusion, autoencoding only once.

3) HF / texture-alignment?

4) Condition on REPA pre-trained features
5) Self-flow baseline
6) Diffuse features together with image?


7) Alternate extension: learn a scale parameter with the tokens, so we can consistently zoom in/out of a particular image.
